...
parser.add_argument('--dataset', dest='dataset', default='HDV', help='which dataset to use')
parser.add_argument('--load_size', dest='load_size', type=int, default=286, help='scale images to this size')
parser.add_argument('--crop_size', dest='crop_size', type=int, default=224, help='then crop to this size')
...
generator_a2a = partial(models.REG_GEN, scope='a2a')
generator_b2b = partial(models.REG_GEN, scope='b2b')
...
a2a = generator_a2a(a_real)
b2b = generator_b2b(b_real)
a2b = generator_a2b(a_real)
b2a = generator_b2a(b_real)
#b2a2b = generator_a2b(b2a)
#a2b2a = generator_b2a(a2b)

a_logit = discriminator_a(a_real, a_real)
b2a_logit = discriminator_a(b2a, a2a)
b_logit = discriminator_b(b_real, b_real)
a2b_logit = discriminator_b(a2b, b2b)
...
reg_loss_a = tf.losses.absolute_difference(a_real, a2a)
reg_loss_b = tf.losses.absolute_difference(b_real, b2b)
#g_loss = g_loss_a2b + g_loss_b2a + cyc_loss_a * 10.0 + cyc_loss_b * 10.0 + reg_loss_a * 10.0 + reg_loss_b * 10.0
g_loss = g_loss_a2b + g_loss_b2a + reg_loss_a * 10.0 + reg_loss_b * 10.0

d_loss_a_real = tf.losses.mean_squared_error(a_logit, tf.ones_like(a_logit))
d_loss_b2a_fake = tf.losses.mean_squared_error(b2a_logit, tf.zeros_like(b2a_logit))
d_loss_a = d_loss_a_real + d_loss_b2a_fake

d_loss_b_real = tf.losses.mean_squared_error(b_logit, tf.ones_like(b_logit))
d_loss_a2b_fake = tf.losses.mean_squared_error(a2b_logit, tf.zeros_like(a2b_logit))
d_loss_b = d_loss_b_real + d_loss_a2b_fake

# summaries
g_summary = utils.summary({g_loss_a2b: 'g_loss_a2b',
                           g_loss_b2a: 'g_loss_b2a',
                           reg_loss_a: 'reg_loss_a',
                           reg_loss_b: 'reg_loss_b'})
...
g_var = [var for var in t_var if 'a2b_generator' in var.name or 'b2a_generator' in var.name or 'b2b_generator' in var.name or 'a2a_generator' in var.name]
...
a2a_pool = utils.ItemPool()
b2b_pool = utils.ItemPool()
...
try:
    ...
    for it in range(sess.run(it_cnt), max_it):
        ...
        a2a_opt, b2b_opt = sess.run([a2a, b2b], feed_dict={a_real: a_real_ipt, b_real: b_real_ipt})

        # train G
        g_summary_opt, _ = sess.run([g_summary, g_train_op], feed_dict={a_real: a_real_ipt, b_real: b_real_ipt})
        summary_writer.add_summary(g_summary_opt, it)
        # train D_b
        d_summary_b_opt, _ = sess.run([d_summary_b, d_b_train_op], feed_dict={a_real: a_real_ipt, b_real: b_real_ipt})
        summary_writer.add_summary(d_summary_b_opt, it)
        # train D_a
        d_summary_a_opt, _ = sess.run([d_summary_a, d_a_train_op], feed_dict={a_real: a_real_ipt, b_real: b_real_ipt})
        ...
        if (it + 1) % 100 == 0:
            ...
            [a2b_opt, b2a_opt] = sess.run([a2b, b2a], feed_dict={a_real: a_real_ipt, b_real: b_real_ipt})
            Gsample_opt = np.concatenate((a_real_ipt, a2b_opt, b_real_ipt, b2a_opt), axis=0)
            [a2a_opt, b2b_opt] = sess.run([a2a, b2b], feed_dict={a_real: a_real_ipt, b_real: b_real_ipt})
            Rsample_opt = np.concatenate((a_real_ipt, a2a_opt, b_real_ipt, b2b_opt), axis=0)
            save_dir = './outputs/sample_images_while_training/' + dataset
            utils.mkdir(save_dir)
            im.imwrite(im.immerge(Gsample_opt, 2, 2), '%s/GEpoch_(%d)_(%dof%d).jpg' % (save_dir, epoch, it_epoch, batch_epoch))
            im.imwrite(im.immerge(Rsample_opt, 2, 2), '%s/REpoch_(%d)_(%dof%d).jpg' % (save_dir, epoch, it_epoch, batch_epoch))
except:
    save_path = saver.save(sess, '%s/Epoch_(%d)_(%dof%d).ckpt' % (ckpt_dir, epoch, it_epoch, batch_epoch))
    print('Model saved in file: % s' % save_path)
    sess.close()
